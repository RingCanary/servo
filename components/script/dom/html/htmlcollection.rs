...
impl HTMLCollectionMethods<crate::DomTypeHolder> for HTMLCollection {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        // Step 1
        let mut result = vec![];
        // Optimization: Use a HashSet to track seen atoms, reducing O(N^2) deduplication to O(N).
        ...
        for elem in self.elements_iter() {
            ...
            if let Some(id_atom) = elem.get_id() {
                // Optimization: Defer DOMString allocation until we know the atom is unique.
