...
impl HTMLCollectionMethods<crate::DomTypeHolder> for HTMLCollection {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        // Step 1
        let mut result = vec![];
        // ⚡ Bolt: Use HashSet for O(1) deduplication instead of O(N) Vec::contains,
        // reducing total time complexity from O(N^2) to O(N).
        ...
        for elem in self.elements_iter() {
            ...
            if let Some(id_atom) = elem.get_id() {
                // ⚡ Bolt: Check uniqueness before allocating DOMString to avoid O(N) heap allocations for duplicates.
