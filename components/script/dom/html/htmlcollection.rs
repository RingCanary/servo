...
impl HTMLCollectionMethods<crate::DomTypeHolder> for HTMLCollection {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        // Step 1
        let mut result = vec![];
        // Optimization: Use HashSet for O(1) deduplication instead of Vec::contains which is O(N)
        ...
        for elem in self.elements_iter() {
            ...
            if let Some(id_atom) = elem.get_id() {
                if seen.insert(id_atom.clone()) {
                    // Optimization: Defer expensive DOMString allocation until after uniqueness check
