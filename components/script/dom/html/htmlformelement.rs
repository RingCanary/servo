...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // ⚡ Bolt: Use native .is_empty() to avoid string allocation overhead (O(1) instead of O(N))
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        // ⚡ Bolt: Pre-allocate capacity and use HashSet for O(1) deduplication instead of O(N^2) Vec search
