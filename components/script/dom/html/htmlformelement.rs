...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // ⚡ Bolt Optimization: `.is_empty()` called on Atom instead of allocating a String first.
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        // ⚡ Bolt Optimization: Use HashSet for O(1) deduplication check,
        // reducing total lookup complexity from O(N^2) to O(N).
        let mut names_vec = Vec::with_capacity(sourced_names_vec.len());
