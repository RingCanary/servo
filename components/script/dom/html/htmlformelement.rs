...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // Optimization: use native `is_empty()` instead of allocating a new string with `.to_string()`
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        let mut names_vec: Vec<DOMString> = Vec::new();
        // Optimization: Use HashSet for O(1) deduplication instead of O(N) Vec::any checks.
