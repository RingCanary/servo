...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // O(1) empty check, avoids heap allocation.
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        // O(N) deduplication using HashSet with pre-allocation.
        let capacity = sourced_names_vec.len();
        let mut names_vec: Vec<DOMString> = Vec::with_capacity(capacity);
