...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // Optimization: Use `name.is_empty()` instead of `name.to_string().is_empty()` to avoid heap allocation
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        // Optimization: Pre-allocate capacity and use a HashSet for O(1) deduplication,
        // reducing the loop complexity from O(N^2) to O(N) by replacing `.iter().any(...)`.
