...
impl HTMLFormElementMethods<crate::DomTypeHolder> for HTMLFormElement {
    ...
    fn SupportedPropertyNames(&self) -> Vec<DOMString> {
        ...
        // Step 6
        // Optimization: Use .is_empty() directly instead of .to_string().is_empty() to prevent String allocation
        sourced_names_vec.retain(|sn| !sn.name.is_empty());

        // Step 7-8
        // Optimization: Pre-allocate capacity to prevent vector reallocations
        let mut names_vec: Vec<DOMString> = Vec::with_capacity(sourced_names_vec.len());
        // Optimization: Use HashSet for O(1) membership checks instead of O(N) Vec::any
        // Overall complexity reduces from O(N^2) to O(N)
        let mut seen = std::collections::HashSet::with_capacity(sourced_names_vec.len());
        for elem in sourced_names_vec.iter() {
            // Optimization: Delay DOMString allocation until after O(1) uniqueness check
